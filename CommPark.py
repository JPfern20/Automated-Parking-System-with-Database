...
from CreateDB import window as cdb
...
# Load the banner image and keep a reference to it
banner_image = ImageTk.PhotoImage(file='banner.ico.png')
# Create a label with the banner image
banner_label = Label(window, image=banner_image)
banner_label.pack(side="top", fill="x")
# Keep a reference to the image to prevent garbage collection
window.banner_image = banner_image

canvas = tk.Canvas(window, width=600, height=800)
canvas.pack(fill="both", expand=True)

image = Image.open("background.ico.png")
background_image = ImageTk.PhotoImage(image)

canvas.create_image(0,0, image=background_image, anchor="nw")

icon = ImageTk.PhotoImage(file='logo.ico')
...
def vehicle_register():
    ...
    check_in_window.config(background='#f1e7e7')
    ...
    Label(check_in_window, text="Vehicle Make:", font=('Arial Narrow', 12, 'bold'), bg='#f1e7e7', fg='black').place(x=50, y=50)
    ...
    Label(check_in_window, text="Vehicle Color:", font=('Arial Narrow', 12, 'bold'), bg='#f1e7e7', fg='black').place(x=50, y=100)
    ...
    Label(check_in_window, text="Vehicle Plate Number:", font=('Arial Narrow', 12, 'bold'), bg='#f1e7e7', fg='black').place(x=50, y=150)
    ...
    Label(check_in_window, text="Vehicle Model:", font=('Arial Narrow', 12, 'bold'), bg='#f1e7e7', fg='black').place(x=50, y=200)
...
def show_customers():
    ...
    customers_window.config(background='#f1e7e7')
    ...
    table_frame = Frame(customers_window, bg='#f1e7e7')
    table_frame.pack(fill=BOTH, expand=True, padx=10, pady=10)

    # Add a title label
    Label(customers_window, text="Customer Details", font=('Arial Narrow', 15, 'bold'), bg='#f1e7e7', fg='black').pack(pady=10)
    ...
    try:
        ...
        if customers:
            ...
            Label(customers_window, text="No Transaction Records found.", font=('Arial Narrow', 12), bg='#f1e7e7', fg='black').pack(pady=20)

    except odbc.Error as e:
        # Handle database errors
        Label(customers_window, text=f"Error fetching data: {e}", font=('Arial Narrow', 12), bg='#f1e7e7', fg='red').pack(pady=20)
...
def check_in():
    ...
    Label(check_in_window, text="Transaction ID:", font=('Arial Narrow', 12, 'bold'), bg='#f1e7e7', fg='black').place(x=50, y=50)
    Label(check_in_window, text=transaction_id, font=('Arial Narrow', 12), bg='#f1e7e7', fg='black').place(x=250, y=50)

    Label(check_in_window, text="Check In Date:", font=('Arial Narrow', 12, 'bold'), bg='#f1e7e7', fg='black').place(x=50, y=100)
    Label(check_in_window, text=check_in_date, font=('Arial Narrow', 12), bg='#f1e7e7', fg='black').place(x=250, y=100)

    Label(check_in_window, text="Vehicle Plate Number:", font=('Arial Narrow', 12, 'bold'), bg='#f1e7e7', fg='black').place(x=50, y=150)
    vehicle_plate_entry = Entry(check_in_window, font=('Arial Narrow', 12), width=30, textvariable=vehicle_plate_var)
    vehicle_plate_entry.place(x=250, y=150)

    Label(check_in_window, text="Floor Slot Number:", font=('Arial Narrow', 12, 'bold'), bg='#f1e7e7', fg='black').place(x=50, y=200)
    ...
    def submit_check_in():
        ...
        if not vehicle_plate or not floor_slot:
            Label(check_in_window, text="All fields are required!", font=('Arial Narrow', 12), bg='#f1e7e7', fg='red').place(x=200, y=300)
            return
        ...
        try:
            ...
            if not vehicle_exists:
                Label(check_in_window, text="Error: Vehicle Plate Number does not exists on the records!", font=('Arial Narrow', 12), bg='#f1e7e7', fg='red').place(x=150, y=300)
                return
            ...
            if not slot_exists:
                Label(check_in_window, text="Error: Floor Slot Number does not exist or is Occupied!", font=('Arial Narrow', 12), bg='#f1e7e7', fg='red').place(x=100, y=300)
                return
            ...
            Label(check_in_window, text="Check-in successful!", font=('Arial Narrow', 12), bg='#f1e7e7', fg='green').place(x=200, y=300)

        except odbc.Error as e:
            print("Error inserting data:", e)
            Label(check_in_window, text=f"Error: {e}", font=('Arial Narrow', 12), bg='#f1e7e7', fg='red').place(x=200, y=300)
        finally:
            on_close()
            cursor.close()
...
def show_vacant_slots():
    ...
    vacant_slots_window.config(background='#f1e7e7')
    
    vacant_slots_window.protocol("WM_DELETE_WINDOW", on_close)
    
    # Add a title label
    Label(vacant_slots_window, text="Vacant Slots", font=('Arial Narrow', 15, 'bold'), bg='#f1e7e7', fg='black').pack(pady=10)
        ...
        if slots:
            ...
            for index, slot in enumerate(slots, start=1):
                ...
                Label(vacant_slots_window, text=f"{index}. Floor Slot Number: {floor_slot_number}",
                      font=('Arial Narrow', 12), bg='#f1e7e7', fg='black').pack(anchor='w', padx=20, pady=5)
        else:
            # Display a message if no slots are vacant
            Label(vacant_slots_window, text="No vacant slots available.", font=('Arial Narrow', 12), bg='#f1e7e7', fg='black').pack(pady=20)

    except odbc.Error as e:
        # Handle database errors
        Label(vacant_slots_window, text=f"Error fetching data: {e}", font=('Arial Narrow', 12), bg='#f1e7e7', fg='red').pack(pady=20)
    ...
    # Add a close button
    Button(vacant_slots_window, text="Close", command=on_close, font=('Arial Narrow', 12), bg='#eb9b34', fg='black').pack(pady=20)
...
def show_vehicles():
    ...
    vehicles_window.config(background='#f1e7e7')

    # Add a title label
    Label(vehicles_window, text="Vehicle List", font=('Arial Narrow', 15, 'bold'), bg='#f1e7e7', fg='black').pack(pady=10)

    # Add a frame to hold the table
    table_frame = Frame(vehicles_window, bg='#f1e7e7')
    table_frame.pack(fill=BOTH, expand=True, padx=10, pady=10)
    ...
    try:
        ...
        if vehicles:
            ...
            Label(vehicles_window, text="No vehicles found.", font=('Arial Narrow', 12), bg='#f1e7e7', fg='black').pack(pady=20)

    except odbc.Error as e:
        # Handle database errors
        Label(vehicles_window, text=f"Error fetching data: {e}", font=('Arial Narrow', 12), bg='#f1e7e7', fg='red').pack(pady=20)
    ...
    # Add a close button
    Button(vehicles_window, text="Close", command=vehicles_window.destroy, font=('Arial Narrow', 12), bg='#eb9b34', fg='black').pack(pady=5)

    # --- Add Delete Vehicle Button ---
    def open_delete_vehicle_window():
        delete_window = Toplevel(vehicles_window)
        delete_window.title("Delete Vehicle")
        delete_window.geometry("400x250")
        delete_window.config(background='#f1e7e7')
        ...
        Label(delete_window, text="Delete Vehicle Record", font=('Arial Narrow', 14, 'bold'), bg='#f1e7e7', fg='black').pack(pady=10)
        Label(delete_window, text="Enter Plate Number to Delete:", font=('Arial Narrow', 12), bg='#f1e7e7', fg='black').pack(pady=5)
        plate_var = StringVar()
        Entry(delete_window, textvariable=plate_var, font=('Arial Narrow', 12), width=25).pack(pady=5)

        warning_text = (
            "WARNING: All records associated with this vehicle\n"
            "(Check-Ins and Payments) will also be deleted!"
        )
        Label(delete_window, text=warning_text, font=('Arial Narrow', 11, 'bold'), bg='#f1e7e7', fg='red').pack(pady=10)

        def confirm_delete():
            plate = plate_var.get().strip()
            if not plate:
                Label(delete_window, text="Plate number is required.", font=('Arial Narrow', 11), bg='#f1e7e7', fg='red').pack(pady=5)
                return
            try:
                cursor = connection.cursor()
                # Delete in correct order: Payments -> Check_Ins -> Vehicles
                cursor.execute("DELETE FROM Payments WHERE Customer_Vehicle_Plate_Number = ?", (plate,))
                cursor.execute("DELETE FROM Check_Ins WHERE Vehicle_Plate_Number = ?", (plate,))
                cursor.execute("DELETE FROM Vehicles WHERE Vehicle_Plate_Number = ?", (plate,))
                connection.commit()
                Label(delete_window, text="Vehicle and associated records deleted.", font=('Arial Narrow', 11), bg='#f1e7e7', fg='green').pack(pady=5)
            except odbc.Error as e:
                Label(delete_window, text=f"Error: {e}", font=('Arial Narrow', 11), bg='#f1e7e7', fg='red').pack(pady=5)
            finally:
                cursor.close()
                delete_window.after(200, delete_window.destroy)  # Close after 2 seconds
                vehicles_window.after(200, vehicles_window.destroy)  # Close after 2 seconds

        Button(delete_window, text="Delete", command=confirm_delete, font=('Arial Narrow', 12), bg='red', fg='white', width=15).pack(pady=10)
        Button(delete_window, text="Cancel", command=on_close, font=('Arial Narrow', 12), bg='#eb9b34', fg='black', width=15).pack(pady=5)

    Button(vehicles_window, text="Delete Vehicle", command=open_delete_vehicle_window, font=('Arial Narrow', 12), bg='red', fg='white').pack(pady=10)
...
def process_payment():
    ...
    payment_window.config(background='#f1e7e7')
    ...
    Label(payment_window, text="Enter Vehicle Plate Number:", font=('Arial Narrow', 12, 'bold'), bg='#f1e7e7', fg='black').place(x=50, y=50)
    ...
    details_frame = Frame(payment_window, bg='#f1e7e7', relief=SOLID, borderwidth=1)
    ...
    def fetch_details():
        ...
        if not Plate_Number:
            for widget in details_frame.winfo_children():
                widget.destroy()
            Label(details_frame, text="Vehicle Plate Number is required!", font=('Arial Narrow', 12), bg='#f1e7e7', fg='red').pack(pady=10)
            return
        ...
        try:
            ...
            if check_in_details:
                ...
                Label(details_frame, text="Vehicle Plate Number not found!", font=('Arial Narrow', 12), bg='#f1e7e7', fg='red').pack(pady=10)

        except odbc.Error as e:
            Label(details_frame, text=f"Error fetching data: {e}", font=('Arial Narrow', 12), bg='#f1e7e7', fg='red').pack(pady=10)
    ...
    def record_payment():
        ...
        if not Vehicle_Plate:
            Label(payment_window, text="Vehicle Plate Number is required!", font=('Arial Narrow', 12), bg='#f1e7e7', fg='red').place(x=250, y=315)
            return
        ...
        try:
            ...
            if not result:
                Label(payment_window, text="Plate Number not found!", font=('Arial Narrow', 12), bg='#f1e7e7', fg='red').place(x=200, y=300)
                return
            ...
            #Create a window to show the payment details
            payment_details_window = Toplevel()
            payment_details_window.title("Payment Details")
            payment_details_window.geometry("400x200")
            payment_details_window.config(background='#f1e7e7')
            payment_details_window.resizable(False, False)

            def on_close():
                payment_details_window.destroy()   
                payment_window.destroy()
                
            # Re-enable the button when the window is closed
            payment_details_window.protocol("WM_DELETE_WINDOW", on_close)

            Label(payment_details_window, text="Payment Details", font=('Arial Narrow', 15, 'bold'), bg='#f1e7e7', fg='black').pack(pady=10)
            Label(payment_details_window, text="Transaction ID: " + str(transaction_id), font=('Arial Narrow', 12), bg='#f1e7e7', fg='black').pack(pady=5)
            Label(payment_details_window, text="Vehicle Plate Number: " + str(vehicle_plate), font=('Arial Narrow', 12), bg='#f1e7e7', fg='black').pack(pady=5)
            Label(payment_details_window, text="Floor Slot Number: " + str(floor_slot), font=('Arial Narrow', 12), bg='#f1e7e7', fg='black').pack(pady=5)
            Label(payment_details_window, text="Transaction Date: " + str(transaction_date), font=('Arial Narrow', 12), bg='#f1e7e7', fg='black').pack(pady=5)
            Label(payment_details_window, text="Amount: " + str(amount), font=('Arial Narrow', 12), bg='#f1e7e7', fg='black').pack(pady=5)
            Label(payment_details_window, text="Status: " + str(status), font=('Arial Narrow', 12), bg='#f1e7e7', fg='black').pack(pady=5)
            Label(payment_details_window, text="Time Out: " + str(time_out), font=('Arial Narrow', 12), bg='#f1e7e7', fg='black').pack(pady=5)

            
            print("Payment Receipt has been issued.")



            # Remove the record from the payments table, check_in table, and then vehicles table
            cursor.execute("DELETE FROM Payments WHERE Customer_Vehicle_Plate_Number = ?", (Vehicle_Plate,))
            cursor.execute("DELETE FROM Check_Ins WHERE Vehicle_Plate_Number = ?", (Vehicle_Plate,))
            cursor.execute("DELETE FROM Vehicles WHERE Vehicle_Plate_Number = ?", (Vehicle_Plate,))
            connection.commit()
            ...
            Label(payment_window, text="Payment recorded successfully!", font=('Arial Narrow', 12), bg='#f1e7e7', fg='green').place(x=246, y=315)
...
button1 = Button(window, text = 'Pending Transaction Records',
...
def Reset_Database():
    ...
    reset_window.config(background='#f1e7e7')
    ...
    Label(reset_window, text="Are you sure you want to reset the database?", font=('Arial Narrow', 12), bg='#f1e7e7', fg='black').pack(pady=10)
    Label(reset_window, text="This action cannot be undone.", font=('Arial Narrow', 12), bg='#f1e7e7', fg='red').pack(pady = 10)
...
canvas.create_text(300, 160, text="Welcome, Please Select an Option", font=("Helvetica", 10, "bold"), fill="#ffd700")


canvas.create_text(300,700, text = f"Status: {IsConnected()} from {get_server_name()}",font=("Helvetica", 10, "bold"), fill = "limegreen")
